...
NCountry = {
	...
	VP_TO_SUPPLY_BONUS_CONVERSION = 0.0,			-- Bonus to supply local supplies from Victory Points, multiplied by this aspect and rounded to closest integer
	SUPPLY_FROM_DAMAGED_INFRA = 3,				-- damaged infrastructure counts as this in supply calcs
	SUPPLY_BASE_MULT = 5,							-- multiplier on supply base values
	SUPPLY_BONUS_FROM_INPUT = 0.25,					-- % of supply bonus from input area.
	SUPPLY_PATH_MAX_DISTANCE = 300,					-- When supply route reach more than X nodes, the manpower+equipment delivery speed reach 100% penalty.
	...
	REINFORCEMENT_DELIVERY_SPEED_MIN = 1,			-- The distance from the supply region to capital should affect the speed only a little bit. Main factor for penalty is overcrowded areas, and not the route length.
	...
	BASE_SURRENDER_LIMIT = 1,						-- Base level of occupation required for country surrender
	...
	STARTING_COMMAND_POWER = 200.0,					-- starting command power for every country
	BASE_MAX_COMMAND_POWER = 1000.0,				-- base value for maximum command power
...
NResistance = {
	...
	RESISTANCE_TARGET_MODIFIER_HAS_CLAIM = -50.0,			-- resistance target modifier in % for states we have claim

	RESISTANCE_TARGET_MODIFIER_PER_STABILITY_LOSS = 0,	-- resistance target modifier per stability below 100%
	RESISTANCE_TARGET_MODIFIER_PER_COMPLIANCE = -5,		-- resistance target modifier per compliance %
	...
	RESISTANCE_DECAY_BASE = 10, -- base resistance decay
	...
	COMPLIANCE_GROWTH_BASE = 10, -- base compliance grow
	COMPLIANCE_GROWTH_MIN = 10.0, -- min compliance grow
...
NProduction = {
	...
	DEFAULT_MAX_NAV_FACTORIES_PER_LINE = 60,
	CONVOY_MAX_NAV_FACTORIES_PER_LINE = 999,
	CAPITAL_SHIP_MAX_NAV_FACTORIES_PER_LINE = 30,
	...
	ANNEX_STOCKPILES_RATIO = 0,		-- How much stockpiled equipment will be transferred on annexation
	ANNEX_FIELD_EQUIPMENT_RATIO = 0,	-- How much equipment from deployed divisions will be transferred on annexation
	ANNEX_FUEL_RATIO = 1,	-- How much fuel will be transferred on annexation
	ANNEX_CONVOYS_RATIO = 1,			-- How many convoys will be transferred on annexation
	...
	CAPITULATE_STOCKPILES_RATIO = 0.0, -- How much equipment from deployed divisions will be transferred on capitulation
...
NBuildings = {
	...
	AIRBASE_CAPACITY_MULT = 1000,		-- Each level of airbase building multiplied by this, gives capacity (max operational value). Value is int. 1 for each airplane.
...
NMilitary = {
	...
	BASE_COMBAT_WIDTH = 160,                        -- base combat width
	ADDITIONAL_COMBAT_WIDTH = 80,                  -- more opened up by support attack
	...
	UNIT_EXP_LEVELS = { 0.1, 0.3, 0.65, 0.8 },		-- Experience needed to progress to the next level
	...
	PARACHUTE_ORG_REGAIN_MULT = -0.8,	           -- penalty to org regain after being parachuted.
	PARACHUTE_ORG_REGAIN_PENALTY_MULT = 0.0,	   -- penalty to org regain after being parachuted.
	...
	AIR_SUPPORT_BASE = 0,                          -- CAS bonus factor for air support moddifier for land unit in combat
	...
	PLAN_PROVINCE_LOW_VP_IMPORTANCE_AREA = 0.5,     -- Used when calculating the value of defense area in the battle plan system
	...
	PLAN_PROVINCE_CAPITAL_IMPORTANCE_AREA = 100.0,	-- Used when calculating the balue of defense area in the battle plan system (Changed from 40 to 100) 
	...
	PLAN_PORVINCE_PORT_BASE_IMPORTANCE = 5.0,		-- Added importance for area defense province with a port
	PLAN_PORVINCE_PORT_LEVEL_FACTOR = 0.1,			-- Bonus factor for port level
	PLAN_PORVINCE_AIRFIELD_BASE_IMPORTANCE = 1.0,	-- Added importance for area defense province with air field
	PLAN_PORVINCE_AIRFIELD_POPULATED_FACTOR = 0.5,	-- Bonus factor when an airfield has planes on it
	PLAN_PORVINCE_AIRFIELD_LEVEL_FACTOR = 0.0,		-- Bonus factor for airfield level
	PLAN_PORVINCE_RESISTANCE_BASE_IMPORTANCE = 0.5, -- Used when calculating the calue of defense area provinces for the battle plan system (factored by resistance level)


	-- These need to result in province value > 1.0 for it to matter.
	PLAN_AREA_DEFENSE_ENEMY_CONTROLLER_SCORE = 20.0,		-- Score applied to provinces in the defense area order controlled by enemies
	...
	PLAN_AREA_DEFENSE_FORT_IMPORTANCE = 10.0,			-- Used when calculating the calue of defense area provinces for the battle plan system, works as multipliers on the rest
	PLAN_AREA_DEFENSE_COASTAL_FORT_IMPORTANCE = 10.0,	-- Used when calculating the calue of defense area provinces for the battle plan system
	PLAN_AREA_DEFENSE_COAST_NO_FORT_IMPORTANCE = 0.1,	-- Used when calculating the calue of defense area provinces for the battle plan system
	...
	PARADROP_HOURS = 4,								-- time paratroopers suffer penalties in combat
	...
	STRATEGIC_REDEPLOY_ORG_RATIO = 0.7,				-- Ratio of max org while strategic redeployment
...
NAir = {
	...
	AIR_WING_MAX_STATS_AGILITY = 200,
...
NNavy = {
	...
	AMPHIBIOUS_INVADE_SPEED_BASE = 1, 							-- every hour movement progress on amphibious invasion
	AMPHIBIOUS_INVADE_MOVEMENT_COST = 24.0, 						-- total progress cost of movement while amphibious invading
	...
	NAVAL_SPEED_MODIFIER = 0.3,	                    				-- basic speed control
...
NAI = {
	...
	LENDLEASE_FRACTION_OF_PRODUCTION = 0,		-- Base fraction AI would send as lendlease
	LENDLEASE_FRACTION_OF_STOCKPILE = 0,		-- Base fraction AI would send as lendlease
	...
	MINIMUM_CONVOY_TO_ASK_LEND_LEASE = 9999,		-- AI will accept to lend lease convoys only if our stockpile is less than that (special case because convoy stockpile can't be negative).
	...
	MAX_SUPPLY_DIVISOR = 1,							-- To make sure the AI does not overdeploy divisions. Higher number means more supply per unit.
